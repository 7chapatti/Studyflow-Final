...
function placeTurn(params: {
  ...
  const {
    from, until, timezone, turnHours, peakPreferenceStrength,
    allowLateNight, respectGap, personalHourWeights, blockedTimes,
    existingBlocks, scheduledSoFar, assignmentDayTotals,
    assignmentDaySessionCounts, dailyAssignmentTarget, globalDayTotals,
    globalDailyTarget, idealNextDate,
  } = params;
  ...
  function slotsFor(gapMinutes: number) {
    ...
    if (!cached) {
      cached = getOpenSlotsByDay({
        from, until, timezone, personalHourWeights, blockedTimes,
        existingBlocks, scheduledSoFar, allowLateNight, gapMinutes,
      });
  ...
  for (const gapMinutes of gapTiers) {
    ...
    for (const dayPreference of ["both", "assignmentOnly", "spacingOnly", "none"] as const) {
      for (const dayKey of orderedDayKeys) {
        ...
        if (dayPreference === "both" || dayPreference === "assignmentOnly") {
...
export function scheduleTasks(userId: string, input: ScheduleInput): ScheduleOutput {
  ...
  for (const task of input.tasks) {
    ...
    if (!tasksByAssignment.has(task.assignment.id)) {
      tasksByAssignment.set(task.assignment.id, []);
    }
  ...
  const sortedAssignments = Array.from(tasksByAssignment.entries())
    .map(([id, tasks]) => ({
      ...
      tasks,
      priority: priorityScore(tasks[0], tasks[0].assignment, now)
    }))
  ...
  for (const group of sortedAssignments) {
    ...
    const totalHours = group.tasks.reduce((sum, t) => sum + (originalHoursToSchedule.get(t.id) ?? 0), 0);
  ...
  while (progressMadeThisPass && passGuard < MAX_PASSES) {
    ...
    for (const group of sortedAssignments) {
      ...
      const placed = placeTurn({
        ...
        existingBlocks: input.existingBlocks, scheduledSoFar,
        assignmentDayTotals: dayTotals, assignmentDaySessionCounts: daySessionCounts,
        dailyAssignmentTarget, globalDayTotals, globalDailyTarget, idealNextDate,
      });
    ...
    if (!anyTaskActive) break;
  }

  for (const task of input.tasks) {
    if (!finished.has(task.id) && task.status !== "done") finalizeAtRisk(task);
  }
