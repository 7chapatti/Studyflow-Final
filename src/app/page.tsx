...
interface CalendarBlockedTime {
  id: string;
  sourceId: string;      // primary source row id (used for single-source ops)
  sourceIds: string[];   // all source row ids that went into this merged block
  label: string;
  startHour: number;
  endHour: number;
  dayIndex: number;
  isMerged: boolean;     // true if 2+ source rows were combined
  type: "blocked";
}
...
export default function CalendarPage() {
  ...
  // One ref per view — desktop and mobile have separate scroll containers
  ...
  const loadData = useCallback(async () => {
    ...
    const rawBT: CalendarBlockedTime[] = [];
    ...
    const mergedBT: CalendarBlockedTime[] = [];
    for (let dayIdx = 0; dayIdx < 7; dayIdx++) {
      const dayBTs = rawBT.filter((bt) => bt.dayIndex === dayIdx).sort((a, b) => a.startHour - b.startHour);
      let i = 0;
      while (i < dayBTs.length) {
        let cur = { ...dayBTs[i] };
        let j = i + 1;
  ...
  function getGridPos(clientX: number, clientY: number, isMobile: boolean) {
    ...
    // On mobile the ref wraps the scrollable area — account for scrollTop
  ...
  function isOccupied(dayIndex: number, s: number, e: number, excludeId: string) {
  ...
  const onPointerUp = useCallback(async () => {
    ...
    if (state.blockType === "task") {
      ...
      if (drop.willMerge) {
        // Dropping onto another blocked time — merge them visually and in DB
        ...
        // Combined range: earliest start, latest end
        ...
        // Update both source sets to the merged range
        ...
        // Update state — replace both blocks with one merged block
  ...
  // Separate a merged blocked time back into individual display blocks
  async function separateBlocks(bt: CalendarBlockedTime) {
    setContextMenu(null);
    if (!bt.isMerged) return;

    // Fetch the original times of each source row from DB
    const supabase = createClient();
  ...
  async function handleReschedule() {
    ...
    try {
      const res = await fetch("/api/schedule/run", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({}) });
      ...
      >
        ...
        {height > 30 && (
          <p className="text-xs truncate opacity-70" style={{ color: textColour }}>
            {pad(block.startTime.getHours())}:{pad(block.startTime.getMinutes())}–{pad(block.endTime.getHours())}:{pad(block.endTime.getMinutes())}
          </p>
        )}
      </button>
    );
